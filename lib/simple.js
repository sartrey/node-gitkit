...
function cloneRepo(cwd, repo) {
  ...
  return assist.safeExecute('git', ['clone', '-q', repo, './'], { cwd, sshkey, ignore: true })
}
 ...
 * @return {Promise}
 */
function resetRepo(cwd, hash) {
  return assist.safeExecute(
    'git', ['reset', '--hard', hash || 'origin/master'],
    { cwd, ignore: true }
  )
}
 ...
 * @return {Promise}
 */
function makeCommit(cwd, msg, user) {
  var cfg = user ?
    `-c user.email="${user.email}" -c user.name="${user.name}"` : ''
  return assist.chainPromise([
    assist.safeExecute('git', ['add', '.'], { cwd, ignore: true }),
    assist.safeExecute('git', [cfg, 'commit', '-m', `"${msg}"`], { cwd, ignore: true })
  ])
}
